...
def password_format(password_str):
	"""
	校验密码格式是否正确
	"""
	if re.match(r"^[A-Za-z0-9]{6,20}$", password_str):

		return password_str
	else:
		raise ValueError('{} is not a valid password you need 密码复杂度最低仅支持字母加数字混合且不得小于6位'.format(password_str))
